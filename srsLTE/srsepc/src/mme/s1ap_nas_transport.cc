                                                     ...
                                                     struct sctp_sndrcvinfo*               enb_sri)
{
  ...
  // EDITIONS BEGIN HERE
  // nas* _nas_ctx = m_s1ap->find_nas_ctx_from_mme_ue_s1ap_id(mme_ue_s1ap_id);
  // srslte::byte_buffer_t* _nas_msg = m_pool->allocate();
  // memcpy(_nas_msg->msg, ul_xport.protocol_ies.nas_pdu.value.data(), ul_xport.protocol_ies.nas_pdu.value.size());
  // _nas_msg->N_bytes   = ul_xport.protocol_ies.nas_pdu.value.size();
  // bool _msg_encrypted = false;
  // liblte_mme_parse_msg_header((LIBLTE_BYTE_MSG_STRUCT*)_nas_msg, &pd, &msg_type);
  // switch (msg_type) {
  ...
  //     _nas_ctx->handle_identity_response(_nas_msg);
  //     break;
  // }
